...
class WuyeBaoGateButton(CoordinatorEntity[WuyeBaoCoordinator], ButtonEntity):
    ...
    async def async_press(self) -> None:
        ...
        # Diagnostic aid: dump the full raw gate record so the correct
        # open-door endpoint / SIP identity can be determined if the default
        # HTTP path is rejected by the server.
        if self._gate_raw is not None:
            _LOGGER.info(
                "门禁原始记录: %s",
                json.dumps(self._gate_raw, ensure_ascii=False, default=str),
            )
