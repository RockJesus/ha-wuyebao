...
# Community selection (the gates endpoint requires a community).
...
API_COMMUNITIES_PATH = "api/owner/anon/owners/community"
...
# Query parameter name carrying the selected community on the gates request.
# The server rejects the request with "必须选择一个小区才能查询信息！" when the
# community is missing. "communityId" is the default; the client automatically
# falls back to "communityCode" if the server rejects the primary name.
DEFAULT_COMMUNITY_FIELD = "communityId"
