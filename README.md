...
诊断方式（v2.3.0）：点击开门按钮时，若默认路径失败，集成会自动再尝试一组
**最可能的 HTTP 开门候选**（用该门禁真实记录构造：带小区参数、密码、设备 uid、
`/open`、`/remoteOpen`、area 前缀路径等），并把**每个候选的状态码和响应**写入 HA 日志
（`开门诊断结果: [...]`）。把这条日志发给维护者即可确定：
- 有 HTTP 候选成功 → 把该路径填到「选项」→ 开门接口路径即可；
- 全部失败（如 404/405）→ 确认该小区只能走 SIP 云对讲，需 SIP 方案。
