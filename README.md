...
3. 登录成功即自动创建门禁按钮和传感器实体。

> 服务器地址、`client_id`、开门接口路径等已内置默认值（来自官方 App），
...
- 若你的物业服务器支持该 HTTP 开门接口 → 按钮直接可用。
- 若服务器不接受（返回 404/参数错误）→ 说明该小区开门必须走 SIP 云对讲，
  需要在 HA 侧部署 SIP 客户端来拨打门禁的 `对讲号码`（门禁传感器属性里可见），
  或联系物业确认是否有业主端 HTTP 远程开门能力。
