...
> 因此：
> - 集成现在会按门禁记录**动态构造本门的 callNumber**（如用户的 4栋1单元27楼
>   门口机 → `RM-840-1-4-1-27-1`），作为 SIP **呼叫目标**（INVITE）和
>   **用户名候选**（REGISTER），密码候选 = 登录密码 / accessToken / 门禁
>   自带密码；
> - 同时新增 HTTP 呼叫类接口探测（`api/call/grant/calls` 等，POST +
>   callNumber），如果服务器有 HTTP 呼叫入口会直接命中。
